...
import { ContributionsTab } from "./(tabs)/accounts";

export default function ContributionsScreen() {
  const insets = useSafeAreaInsets();
  const webTopInset = Platform.OS === "web" ? webTopInsetBase : 0;
  const { contributionYTD, contributionLimit, transactions, addContribution } = useHSA();
    ...
    <View style={[styles.container, { paddingTop: insets.top + webTopInset }]}>
      <View style={styles.header}>
        <Pressable onPress={() => router.back()} style={styles.backBtn}>
          <Feather name="arrow-left" size={28} color={Colors.light.text} />
        </Pressable>
